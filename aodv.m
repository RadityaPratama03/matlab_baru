...
for t = 1:100
    ...
    group.ResultNormal{t} = resultTable;
...
for t = 1:100
    ...
    resultTableTime = group.ResultNormal{t};
    ...
    group.ResultNormal{t} = resultTableTime;
...
for t = 1:100
    % Mengambil tabel dari dalam cell array
    resulttime = group.ResultNormal{t};
    ...
    % Mengatur semua nilai dalam rt menjadi 40
    rt = repmat(15, height(resulttime), 1);
    ...
    % Menyimpan tabel yang telah dimodifikasi ke dalam cell array
    group.ResultSerangan{t} = resulttime;
...
for t_idx = 1:2
    ...
    resulttime = group.ResultSerangan{t_idx};
...
%     title('Throughput');
    ...
    resultTableTime = group.ResultNormal{t_idx};
    ...
    minD = min(resulttime.d(~strcmp(resulttime.color, 'red')));
    newHeadClusterIndex = find(resulttime.d == minD, 1);

    for i = 1:size(resulttime, 1)
%         if i == newHeadClusterIndex
        if strcmp(resulttime.color{i}, 'Head Cluster')
            figure(2);
    ...
    total_pt_1 = sum(group.ResultNormal{t_idx}.pt);
    total_rt_1 = sum(group.ResultNormal{t_idx}.rt);
%     Delay1 = total_pt_1 / max(total_rt_1, 1);
    Delay1 = total_pt_1 / total_rt_1;
    
    % Perhitungan throughput pada detik t_idx untuk group.Result
    paket_diterima_1 = group.ResultNormal{t_idx}.rt; % paket data yang diterima dalam kb
    waktu_pengiriman_1 = group.ResultNormal{t_idx}.pt; % waktu pengiriman dalam detik
    Throughput1 = paket_diterima_1 ./ max(waktu_pengiriman_1, 1);
    
    % Perhitungan delay pada detik t_idx untuk group.ResultTime
    total_pt_2 = sum(group.ResultSerangan{t_idx}.pt);
    total_rt_2 = sum(group.ResultSerangan{t_idx}.rt);
%     Delay2 = total_pt_2 / max(total_rt_2, 1);
    ...
    paket_diterima_2 = group.ResultSerangan{t_idx}.rt; % paket data yang diterima dalam kb
    waktu_pengiriman_2 = group.ResultSerangan{t_idx}.pt; % waktu pengiriman dalam detik
...
%     resultTableTimeCurrent = group.ResultSerangan{t};
%     resultTableTimeNext = group.ResultSerangan{t + 1};
%     

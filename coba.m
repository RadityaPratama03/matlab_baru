...
for i = 1:length(Data_t)
    ...
    data_xy_angle = [x, y, normalized_angle];
    k = 4; % Jumlah cluster yang diinginkan
    [idx_kmeans, C] = kmeans(data_xy_angle, k);
    ...
    for cluster = 1:k
        ...
        scatter(cluster_points(:, 1), cluster_points(:, 2), 50, color, marker, 'filled');
        hold on;
    
        % Plot centroid cluster dengan tanda bintang
        scatter(C(cluster, 1), C(cluster, 2), 200, color, 'X', 'LineWidth', 2);
    end
    
    % Menambahkan label pada plot
    xlabel('Data x');
    ylabel('Data y');
    title('Hasil Clustering K-Means (4 Cluster) dengan Data Sudut yang Dinormalisasi');

    % Menampilkan legenda
    legend('Cluster 1', 'Centroid Cluster 1', 'Cluster 2', 'Centroid Cluster 2', 'Cluster 3', 'Centroid Cluster 3', 'Cluster 4', 'Centroid Cluster 4', 'Location', 'northwest');
